...
- `scripts/` 包含全部可执行逻辑：`publish_pipeline.py`（主流程）、`cdp_publish.py`（CDP 自动化与账号命令）、`chrome_launcher.py`（Chromium 生命周期管理）以及辅助模块（`image_downloader.py`、`account_manager.py`、`run_lock.py`）。
...
- `python scripts/chrome_launcher.py`：启动启用 CDP 的 Chromium（有界面）。
...
## 安全与配置建议
- 禁止提交真实 Cookie、账号令牌或个人 Chromium Profile 路径。
